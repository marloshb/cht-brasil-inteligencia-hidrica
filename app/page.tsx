...
export default function Home() {
  ...
  useEffect(() => {
    if (moduleId === "m1" || moduleId === "m2" || moduleId === "m3") return;
  ...
  const showSpecialistMap = showIdentityMap || showPassportMap || showRegulatoryMap;
  const hideWorkspace = (moduleId === "m0" && !["Visão nacional", "Mapa operacional"].includes(contextItem)) || (moduleId === "m1" && !showIdentityMap) || (moduleId === "m2" && !showPassportMap) || (moduleId === "m3" && !showRegulatoryMap);
  ...
  return (
    <div className="app-shell" style={{ "--module-accent": activeModule.accent } as React.CSSProperties}>
      ...
      <main className="main-content">
        ...
        <section className={`workspace-grid ${hideWorkspace ? "generic-hidden" : ""} ${showSpecialistMap ? "identity-map-only" : ""}`}>
          <article className="panel map-panel">
            ...
            <footer className="selection-strip">
              ...
              <div><small>SELEÇÃO ATIVA</small><strong>{["m1", "m2", "m3"].includes(moduleId) ? mapFocus.label : currentEvent.title}</strong><span>{["m1", "m2", "m3"].includes(moduleId) ? `${mapFocus.source} · confiança ${(mapFocus.confidence / 100).toFixed(2).replace(".", ",")}` : `${currentEvent.source} · confiança ${journeyStep > 2 ? "0,92" : "0,88"}`}</span></div>
              <button onClick={() => moduleId === "m1" ? setContextItem("Versões") : moduleId === "m2" ? setContextItem("Regularidade") : moduleId === "m3" ? setContextItem("Competências") : setDecisionOpen(true)}>{moduleId === "m1" ? "Abrir identidade →" : moduleId === "m2" ? "Abrir passaporte →" : moduleId === "m3" ? "Abrir competência →" : "Abrir dossiê →"}</button>
            </footer>
          </article>
        ...
        <section className={`analytics-grid ${(moduleId === "m0" && contextItem !== "Visão nacional") || moduleId === "m1" || moduleId === "m2" || moduleId === "m3" ? "generic-hidden" : ""}`}>
        ...
        <section className={`panel cases-panel ${moduleId === "m0" || moduleId === "m1" || moduleId === "m2" || moduleId === "m3" ? "generic-hidden" : ""}`}>
        ...
        <section className={`module-blueprint panel ${moduleId === "m0" || moduleId === "m1" || moduleId === "m2" || moduleId === "m3" ? "generic-hidden" : ""}`}>
