...
export function PassportHub({ contextItem, territory, clockLabel, onNavigate, onOpenModule, onCreateRecord, onToast }: PassportHubProps) {
  ...
  const routeObligation = (obligation: Obligation) => {
    ...
    onOpenModule(obligation.module);
  ...
  const EvidenceVault = () => <div className="passport-evidence">
    ...
    <div className="passport-evidence-policy"><span>POLÍTICA DE EVIDÊNCIA</span><p>O bruto é preservado, enriquecimentos criam novas versões e nenhuma evidência expirada é apagada. Dados sensíveis seguem mascaramento e finalidade.</p><button onClick={() => onOpenModule("m11")}>Abrir governança →</button></div>
  </div>;
